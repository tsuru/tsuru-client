...
func ReadTarget() (string, error) {
	...
	if err == errUndefinedTarget {
		copyTargetFiles()
		target, err = readTarget(config.JoinWithUserDir(".tsuru_target"))
	}
	return target, err
}

func readTarget(targetPath string) (string, error) {
	if f, err := config.Filesystem().Open(targetPath); err == nil {
		defer f.Close()
...
func deleteTargetFile() {
	config.Filesystem().Remove(config.JoinWithUserDir(".tsuru", "target"))
}
...
func getTargets() (map[string]string, error) {
	...
	var legacy bool
	f, err := config.Filesystem().Open(targetsPath)
	if os.IsNotExist(err) {
		f, err = config.Filesystem().Open(legacyTargetsPath)
		legacy = true
	}
	if err == nil {
		defer f.Close()
...
func copyTargetFiles() {
	...
	if target, err := readTarget(config.JoinWithUserDir(".tsuru_target")); err == nil {
		WriteTarget(target)
	}
}
