...
func readRecordedTarget(fs *fstest.RecordingFs) string {
	filePath := config.JoinWithUserDir(".tsuru", "target")
...
func (s *S) TestReadTargetLegacy(c *check.C) {
	...
	f, err := rfs.Create(config.JoinWithUserDir(".tsuru_target"))
	c.Assert(err, check.IsNil)
	...
	c.Assert(err, check.IsNil)
	c.Assert(target, check.Equals, "http://tsuru.google.com")
	target, err = readTarget(config.JoinWithUserDir(".tsuru", "target"))
	c.Assert(err, check.IsNil)
	c.Assert(target, check.Equals, "http://tsuru.google.com")
	dir := config.JoinWithUserDir(".tsuru")
...
func (s *S) TestDeleteTargetFile(c *check.C) {
	...
	deleteTargetFile()
	targetFile := config.JoinWithUserDir(".tsuru", "target")
...
func (s *S) TestGetURL(c *check.C) {
	...
	for _, t := range tests {
		config.SetFileSystem(&fstest.RecordingFs{FileContent: t.target})
...
func (s *S) TestTargetAddRun(c *check.C) {
	...
	context := &cmd.Context{
		Args:   []string{"default", "http://tsuru.google.com"},
...
func (s *S) TestGetTargetsLegacy(c *check.C) {
	...
	f, err := rfs.Create(config.JoinWithUserDir(".tsuru_targets"))
	c.Assert(err, check.IsNil)
	...
	c.Assert(err, check.IsNil)
	c.Assert(got, check.DeepEquals, expected)
	f, err = rfs.Open(config.JoinWithUserDir(".tsuru", "targets"))
	c.Assert(err, check.IsNil)
	defer f.Close()
	b, err := io.ReadAll(f)
	c.Assert(err, check.IsNil)
...
func (s *S) TestTargetListRun(c *check.C) {
	os.Unsetenv("TSURU_TARGET")
	content := `first	http://tsuru.io
default	http://tsuru.google.com
other	http://other.tsuru.io`
	rfs := &fstest.RecordingFs{}
	f, _ := rfs.Create(config.JoinWithUserDir(".tsuru", "target"))
	f.Write([]byte("http://tsuru.io"))
	f.Close()
	f, _ = rfs.Create(config.JoinWithUserDir(".tsuru", "targets"))
