...
func (s *S) TestReadTargetLegacy(c *check.C) {
	...
	f, err := rfs.Create(JoinWithUserDir(".tsuru_target"))
	c.Assert(err, check.IsNil)
...
func (s *S) TestGetURL(c *check.C) {
	...
	for _, t := range tests {
		SetFileSystem(&fstest.RecordingFs{FileContent: t.target})
...
func (s *S) TestGetTargetsLegacy(c *check.C) {
	...
	f, err := rfs.Create(JoinWithUserDir(".tsuru_targets"))
	c.Assert(err, check.IsNil)
	...
	c.Assert(err, check.IsNil)
	c.Assert(got, check.DeepEquals, expected)
	f, err = rfs.Open(JoinWithUserDir(".tsuru", "targets"))
	c.Assert(err, check.IsNil)
	defer f.Close()
	b, err := io.ReadAll(f)
	c.Assert(err, check.IsNil)
